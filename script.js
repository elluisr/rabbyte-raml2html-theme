...
$(document).ready(function () {
	$("#searchbar").keyup(function (event) {
		...
		$(".nav-bar-resources").each(function (index) {

			let urlNames = $(this).text().split(" ");

			if (urlNames[2].indexOf(value) != -1) {
	...
	$(".nav-bar-resources").click(function (event) {
		let url_value = $(this).text().split(" ")[2];
		...
		$(".record").each(function (index) {
			...
			if ($(this).text().split(" ")[1].indexOf(url_value) != -1) {
